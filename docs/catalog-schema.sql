-- Device Lover catalog DB design, PostgreSQL 16, 2026-09-21.
-- DESIGN ONLY: not a sqlx migration and not applied to any database.
-- Public API contracts: catalog-openapi.yaml. Business rules: catalog-design.md.
-- Existing users/auth tables are independent and are not changed here.
BEGIN;
...
-- One comparison target = one model in one market, not a selected storage SKU.
-- KR smartphones are the first supported catalog; other markets/categories need
-- their own spec registry and identifier namespace policy before these checks expand.
...
-- Original labels are retained for API aliases[] and modelNumbers[].
-- Slug and current display name remain on device_models.
...
-- Derived lookup registry for slug + name + all aliases. Different original
-- labels of the SAME model may normalize to the same route_key: insert once.
-- A route_key can never resolve to two different models. Rebuild atomically
-- with model/alias writes; a collision aborts the entire transaction.
-- NFKC/case handling is done by the application, not by a database collation.
...
-- The B-tree covers exact search; MVP prefix/substring matching may scan the
-- small registry. Introduce pg_trgm only after measuring actual catalog scale.

-- Only source-verified, purchasable RAM/storage combinations belong here.
-- Never generate the cartesian product of memory.optionsGb and storage.optionsGb.
...
-- At most one is enforced here; at least one checked primary source is required
-- by the publication service before a model can become/remain published.
CREATE UNIQUE INDEX device_sources_one_primary_idx
    ON device_sources (device_id) WHERE is_primary;

-- Fixed smartphone keys; this is not an arbitrary administrator-defined EAV.
-- Display copy is retained alongside typed raw values while the current table
-- migrates away from parsing units out of Korean display strings.
...
-- Intentionally service-level, not claimed as database CHECK guarantees:
-- * Validate key-specific raw_value schemas against the API/domain registry.
-- * A published model has exactly 27 spec rows, all expected identifiers,
--   one checked primary source, and checked sources for all source_id overrides.
-- * Validate configurations against model option sets and official sources.
-- * For ANY child change, lock the model, revalidate publication invariants,
--   and update device_models.updated_at in the same transaction.
-- * Preserve retired public identifiers as aliases; archive published models.
-- * Raw data, display text, and detail are validated/updated together.
COMMIT;
